...
export class Parser {
   ...
   */
  protected parseLocale(data: Record<string, string>, isDefaultLanguage = false): TranslateTypeConfig[] {
  ...
  parse(locales: LocaleJSON[]): TranslateTypeConfig[] {
    const arr = locales.flatMap((value) =>
      this.parseLocale(value.data, path.basename(value.path).includes(this.language)),
    )
    return uniqueBy(
      sortBy(arr, (config) => (config.value === 'string' || config.value === undefined ? 1 : 0)),
      (config) => config.key,
    )
  }
}

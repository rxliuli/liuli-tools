...
describe('测试 Generator', () => {
  ...
  let cwd: string
  beforeAll(async () => {
    cwd = (await findParent(__dirname, (dir) => pathExists(path.resolve(dir, 'package.json'))))!
  ...
  it('测试 parseChangeLog', async () => {
    const res = Generator.parseChangeLog(await readFile(path.resolve(cwd!, 'CHANGELOG.md'), 'utf-8'))
    console.log(res)
  })
    ...
    const res = Generator.stringifyChangeLog([
      {
        ...
        version: '0.1.0',
        contents: ['feat: 初始化 @liuli-util/liuli-cli-plugin-monorepo-changelog 模块5'],
      },
    ])
    console.log(res)
  })

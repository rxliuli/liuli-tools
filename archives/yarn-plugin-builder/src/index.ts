 ...
 */
export async function buildYarnPlugin(options: YarnPluginBuildOptions): Promise<void> {
  ...
  await build({
    ...
    bundle: true,
    outfile: path.resolve(options.base, `bundles/${options.org}/${options.name}.js`),
    logLevel: `silent`,
    format: `iife`,
    platform: `node`,

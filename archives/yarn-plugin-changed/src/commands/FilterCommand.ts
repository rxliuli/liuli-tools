...
import { CacheWorkspace, listChangedWorkspaces } from '../utils/listChangedWorkspaces'
import { FilterOptions, filterWorkspaces } from '../utils/filterWorkspaces'
import ora from 'ora'

export abstract class FilterCommand extends BaseCommand implements FilterOptions {
  ...
  async listWorkspaces(project: Project): Promise<CacheWorkspace> {
    ...
    const exclude = this.exclude || [structUtils.stringifyIdent(project.getWorkspaceByCwd(project.cwd).locator)]
  ...
  async main(): Promise<CacheWorkspace | null> {
    ...
    spinner.stopAndPersist({
      text: `计算得到变更的模块: 
${changed.workspaces.map((item) => structUtils.stringifyIdent(item.locator)).join('\n')}`,
    })

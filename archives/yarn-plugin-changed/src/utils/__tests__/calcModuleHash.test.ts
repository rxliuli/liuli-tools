...
import { calcModuleHash, getLastCommit, getStatus, git } from '../calcModuleHash'
...
describe('测试获取模块中变更的文件', () => {
  ...
  afterEach(() => remove(testFilePath))
  it('测试没有变更的文件', async () => {
    expect((await getStatus(path.resolve(__dirname, 'asset/test-delete.json'))).length === 0).toBeTruthy()
  })
  it('新建的文件', async () => {
...
describe('测试获取模块的变更', () => {
  ...
  it('测试真实模块', async () => {
    const realModulePath = path.resolve('C:\\Users\\rxliuli\\Code\\company\\matrix\\apps\\spacetop')
    await git.cwd(realModulePath)
    const res = await calcModuleHash(realModulePath)
    console.log(res)
  })
  it('测试 path.resolve', () => {
    const res = path.resolve('C:\\Users\\rxliuli\\Code\\Pkg\\liuli-tools', 'apps/liuli-cli/package.json')
    console.log(res)
  })
})

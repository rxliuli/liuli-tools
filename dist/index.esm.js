 ...
 class InnerBaseAsyncArray {
   ...
   */
   async every(
    ...
    return (await this.findIndex(CombinedPredicate.not(fn) )) === -1
  }
  /**
   * 异步的 some
   * @param fn 异步匹配函数
...
class AsyncArray {
   ...
   * @param args 任意个参数
   */
  constructor(...args) {AsyncArray.prototype.__init.call(this);
...
function watchEventListener() {
  ...
  class Event {
    constructor(
       ...
       listener,
       useCapture,
    ) {this.el = el;this.type = type;this.listener = listener;this.useCapture = useCapture;}
  }
  /**
 ...
 */
class StateMachine {constructor() { StateMachine.prototype.__init.call(this); }
  /**
   * 获取到一个状态工厂
...
function format(str, args) {
  return Object.keys(args).reduce(
    (res, k) => res.replace(new RegExp(`{${k}}`, 'g'), toString$1(args[k])),
    str,
  )
}
 ...
 */
class PubSubMachine {constructor() { PubSubMachine.prototype.__init.call(this); }
  /**
   * 订阅者集合
   */
   __init() {this.subMap = new Map();}
  /**
   * 发布一个主题
   * @param topic 发布的主题
 ...
 */
class MicrotaskQueue {constructor() { MicrotaskQueue.prototype.__init.call(this);MicrotaskQueue.prototype.__init2.call(this); }
  // task 列表
   __init() {this.tasks = [];}
  // 当前是否存在正在执行的 task
   __init2() {this.lock = false;}
  add(func) {
...
class WebStorage  {
  constructor( storage) {this.storage = storage;WebStorage.prototype.__init.call(this);WebStorage.prototype.__init2.call(this);WebStorage.prototype.__init3.call(this);WebStorage.prototype.__init4.call(this);WebStorage.prototype.__init5.call(this);}
  ...
  __init2() {this.getItem = (key) => this.storage.getItem(key);}

  __init3() {this.key = (index) => this.storage.key(index);}

  __init4() {this.removeItem = (key) => {
    this.storage.removeItem(key);
  };}

  __init5() {this.setItem = (key, value) => {

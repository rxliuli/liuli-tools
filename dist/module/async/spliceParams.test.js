...
describe('test spliceParams', () => {
    ...
    it('test spliceParams for Date type', () => {
        ...
        expect(spliceParams({
            birthday: date,
        })).toBe(`birthday=${encodeURIComponent(dateFormat(date, 'yyyy-MM-ddThh:mm:ss.SSS'))}&`);
    });
    ...
    it('test spliceParams for other type', () => {
        const str = 'test';
        expect(() => spliceParams(str)).toThrowError(`The parameter type must be Object: ${str}`);
    });
    it('test spliceParams for array Date', () => {
        ...
        expect(spliceParams({
            name: 'rx',
            bent: [date, date],
        })).toEqual(`name=rx&bent=${encodeURIComponent(dateFormat(date, 'yyyy-MM-ddThh:mm:ss.SSS'))}&bent=${encodeURIComponent(dateFormat(date, 'yyyy-MM-ddThh:mm:ss.SSS'))}&`);
    });
});

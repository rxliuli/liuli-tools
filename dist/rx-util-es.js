 ...
 * @returns 压平一层的数组
 */
function flatMap(arr, fn = v => Array.from(convert(v))) {
 ...
 */
function onceOfSameParam(fn, paramConverter = (...args) => JSON.stringify(args)) {
 ...
 */
const TimeoutInfinite = 'TimoutInfinite';
...
class LocalStorageCache extends ICache {
    ...
    clearExpired() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield delKeys.forEach((key) => __awaiter(this, void 0, void 0, function* () { return local.removeItem(key); }));
        });
    }
    /**
     * 根据 key + value 添加
    ...
    set(key, val, cacheOption = {}) {
        ...
        this.localStorage.setItem(key, JSON.stringify(new CacheVal({
            ...
            cacheOption: Object.assign({}, option, { timeStart: option.timeStart || Date.now() }),
        })));
    }
    /**
     * 根据 key 获取
    ...
    get(key, cacheOption = {}) {
        ...
        const { timeStart, timeout, deserialize } = assign(this.cacheOption, cacheVal.cacheOption, cacheOption);
        // 如果超时则删除并返回 null
        if (timeout !== TimeoutInfinite && Date.now() - timeStart > timeout) {
            this.del(key);
            return null;
...
export { AsyncArray, CombinedPredicate, DateFormatter, FetchLimiting, LocalStorageCache, Locker, StateMachine, StringStyleType, StringStyleUtil, aggregation, antiDebug, appends, arrayDiffBy, arrayToMap, arrayValidator, asIterator, assign, async, asyncFlatMap, asyncLimiting, autoIncrement, blankToNull, blankToNullField, bridge, cacheUtil, compare, compose, copyText, createElByString, curry, dateBetween, dateConstants, dateEnhance, dateFormat, dateParse, debounce, deepFreeze, deepProxy, deletes, deny, download, downloadString, downloadUrl, emptyAllField, emptyFunc, excludeFields, excludeFieldsDeep, fetchTimeout, fill, filterItems, findIndex, flatMap, floatEquals, formDataToArray, format, getCookies, getCusorPostion, getObjectEntries, getObjectKeys, getYearWeek, groupBy, insertText, isBlank, isEditable, isEmpty, isFloat, isNullOrUndefined, isNumber, isRange, lastFocus, listToTree, loadResource, loadScript, logger, mapToObject, nodeBridgeUtil, objToFormData, objectToMap, once, onceOfSameParam, parseUrl, pathUtil, randomInt, range, readLocal, removeEl, removeText, returnItself, returnReasonableItself, safeExec, setCusorPostion, sets, singleModel, sleep, sortBy, spliceParams, strToArrayBuffer, strToDate, stringValidator, throttle, timing, toLowerCase, toObject, toString, toUpperCase, treeMapping, treeToList, trySometime, trySometimeParallel, uniqueBy, wait, waitResource, watch, watchEventListener, watchObject };

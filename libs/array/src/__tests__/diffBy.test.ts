...
describe('test diffBy', () => {
  ...
  it('custom kFn', () => {
    ...
    const kFn = (v: Symbol) => v.toString()
    const { left, right, common } = diffBy(thanArr, thatArr, kFn)
    expect(left.map(kFn)).toEqual(['Symbol(1)', 'Symbol(4)'])
    expect(right.map(kFn)).toEqual(['Symbol(5)'])
    expect(common.map(kFn)).toEqual(['Symbol(2)', 'Symbol(3)'])
  })
  it('custom diff type', () => {
    interface IUser {
      name: string
    ...
    const { common } = diffBy(userList, personList, (item) => item.name)

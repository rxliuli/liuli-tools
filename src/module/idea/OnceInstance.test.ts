...
describe('测试 OnceInstance', () => {
  function testOnce(basicOnce: BasicOnce) {
    const { once, onceOfSameParam } = basicOnce
    describe(`测试 ${
      Reflect.getPrototypeOf(basicOnce).constructor.name
    }`, () => {
      describe('测试 once', () => {
        it('简单示例 +1', () => {
